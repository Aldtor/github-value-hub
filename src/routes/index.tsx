...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Look up any GitHub profile, see growth charts, tweak the valuation formula, and export a PDF report." },
...
function valuate(u: GhUser, a: ReturnType<typeof aggregate>, w: Weights) {
  ...
  const value = Object.values(parts).reduce((s, v) => s + v, 0);
  return { value: Math.round(value), parts };
}

function buildGrowthSeries(u: GhUser, repos: Repo[]) {
  // Cumulative stars + repos count over time, bucketed by year of repo creation.
  ...
  // Estimated followers curve scaled to current followers across the same span.
  // GitHub doesn't expose historical follower counts; we model an S-curve based on cumulative stars.
  const maxStars = cumStars || 1;
  return points.map(p => ({
    ...p,
    followersEst: Math.round((p.stars / maxStars) * u.followers),
  }));
}
      ...
      <main className="max-w-5xl mx-auto px-6 pb-20">
        <section className="text-center pt-12 pb-10">
          ...
          <p className="mt-5 text-muted-foreground max-w-xl mx-auto text-lg">
            Drop any GitHub username. See growth charts, tweak the formula, export a PDF.
          </p>
            ...
            <div className="md:col-span-3 grid md:grid-cols-2 gap-5">
              ...
              <ChartCard title="Followers growth (estimated)">
                ...
                <p className="text-[10px] text-muted-foreground mt-2 font-mono">
                  GitHub doesn't expose historical followers — curve is modeled from cumulative stars scaled to current followers ({user.followers}). Blue line: repos.
                </p>
              </ChartCard>
            </div>
            ...
            <div className="md:col-span-3 p-6 rounded-2xl border border-border" style={{ background: "var(--gradient-card)" }}>
              ...
              <div className="grid md:grid-cols-2 gap-x-8 gap-y-4">
                {WEIGHT_META.map(({ key, label, max, step }) => {
                  const w = weights[key];
                  const contribution = Math.round(val.parts[key]);
                  return (
                    <div key={key}>
